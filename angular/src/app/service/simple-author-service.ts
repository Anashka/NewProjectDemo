...
export class SimpleauthorService implements AuthorService {
  ...
  addauthor(author: Author): boolean {
    if (author && author.id && author.name) {
      console.log("Checking frontend:",author);
      this.http
  ...
  getauthors(): any {
    this.http
      .get<{ message: string; result: Author[] }>(
        'http://localhost:5000/api/author/findall'
      )
  ...
  getauthor(id: string) {
    this.http
      .get('http://localhost:5000/api/author/find/' + id)
      .subscribe((output) => {
        console.log(output);
      });
  ...
  removeauthor(id: string) {
    const newList = [];
    this.http
      .delete('http://localhost:5000/api/author/removeauthor/' + id)
      .subscribe((output) => {
        console.log(output);
        // this.update.next([...this.authors]);
      });
